...
- (void)showWhatsNew {
    ...
    if (![[USER_DEFAULTS objectForKey:@"whatsnew"] isEqual:_appBuild]) {
        ...
        //Setting for constraints (justin)
        [self->_overlay setTranslatesAutoresizingMaskIntoConstraints:NO];
        //distance between child and parent view (top)
        NSLayoutConstraint *contraint1 = [NSLayoutConstraint constraintWithItem:self->_overlay attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.tabBarController.view attribute:NSLayoutAttributeTop multiplier:1.0 constant:0];
        //distance between child and parent view (left)
        NSLayoutConstraint *contraint2 = [NSLayoutConstraint constraintWithItem:self->_overlay attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.tabBarController.view attribute:NSLayoutAttributeLeft multiplier:1.0 constant:0];
        //distance between child and parent view (bottom)
        NSLayoutConstraint *contraint3 = [NSLayoutConstraint constraintWithItem:self->_overlay attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.tabBarController.view attribute:NSLayoutAttributeBottom multiplier:1.0 constant:0];
        //distance between child and parent view (right)
        NSLayoutConstraint *contraint4 = [NSLayoutConstraint constraintWithItem:self->_overlay attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.tabBarController.view attribute:NSLayoutAttributeRight multiplier:1.0 constant:0];
        //add contraints
        NSArray *array = [NSArray arrayWithObjects:contraint1, contraint2, contraint3, contraint4, nil];
        [self.tabBarController.view addConstraints:array];
    }
}
...
- (IBAction)actionsPressed:(id)sender {
    ...
    UIAlertAction *actionWhatsNew = [UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"whats_new", [UTIL getLanguage], @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        self->_overlay = [[WhatsNewView alloc] init];
        
        //add backgroundColor and constraints (justin)
        self->_overlay.backgroundColor = [UTIL blueColor];
